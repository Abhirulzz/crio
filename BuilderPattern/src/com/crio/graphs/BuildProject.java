...
public class BuildProject {
	...
	private static boolean dfs(String node, Set<String> pathVis, Set<String> vis, Stack<String> st,
		...
		for (String it : adj.get(node)) {
			...
			if (!vis.contains(it)) {
				if (dfs(it, pathVis, vis, st, adj, proj))
					return true;
			} else

			if (pathVis.contains(it)) {
				return true;
			}
	...
	static ArrayList<String> topoSort(ArrayList<String> proj, List<List<String>> edges) {
		...
		Set<String> pathVis = new HashSet<>();

		Stack<String> st = new Stack<String>();
		for (int i = 0; i < V; i++) {
			if (!vis.contains(proj.get(i))) {
				if (dfs(proj.get(i), pathVis, vis, st, adj, proj))
					hasCycle = true;
			}
		}
